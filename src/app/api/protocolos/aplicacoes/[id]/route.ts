...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const atual = await prisma.protocoloAplicacao.findUnique({ where: { id: params.id }, include: { protocolo: true, paciente: { select: { unidadeId: true } } } });
  if (!atual || !unidadeOk(s, atual.paciente.unidadeId)) return NextResponse.json({ erro: "Aplicação não encontrada" }, { status: 404 });
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  if (!s) return NextResponse.json({ erro: "Não autenticado" }, { status: 401 });
  if (!podeEscrever(s.role)) return NextResponse.json({ erro: "Sem permissão" }, { status: 403 });
  const alvo = await prisma.protocoloAplicacao.findUnique({ where: { id: params.id }, include: { paciente: { select: { unidadeId: true } } } });
  if (!alvo || !unidadeOk(s, alvo.paciente.unidadeId)) return NextResponse.json({ erro: "Não encontrada" }, { status: 404 });
