...
import { ShieldCheck, Pencil, Trash2, FileStack, PenLine, History } from "lucide-react";
import { ROTULO_TIPO_REGISTRO } from "@/lib/validators/prontuario";
import { formatarData } from "@/lib/utils";

interface Registro {
  id: string; tipo: string; conteudo: string; assinado: boolean;
...
export function RegistroCard({ registro }: { registro: Registro }) {
  ...
  async function salvarEdicao() {
    setOcupado(true);
    const res = await fetch(`/api/prontuario/${registro.id}`, {
      method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ conteudo: texto }),
    });
    setOcupado(false);
  ...
  async function salvarNovaVersao() {
    setOcupado(true);
    const res = await fetch(`/api/prontuario/${registro.id}/nova-versao`, {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ conteudo: texto }),
    });
    setOcupado(false);
  ...
  return (
    <article className="rounded-xl border bg-surface p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        {registro.assinado ? (
          <span className="flex items-center gap-1 rounded-lg bg-emerald-500/15 px-2 py-1 text-xs font-medium text-emerald-600" title={`Selo: ${registro.assinaturaHash}`}>
      ...
      {editando ? (
        <div className="mt-3">
          <textarea value={texto} onChange={(e) => setTexto(e.target.value)} rows={6} className={ta} />
          <div className="mt-2 flex gap-2">
            <button onClick={salvarEdicao} disabled={ocupado} className={btnPrim}>Salvar</button>
            <button onClick={() => { setEditando(false); setTexto(registro.conteudo); }} className={btnSec}>Cancelar</button>
          </div>
        </div>
      ) : (
      ...
      {novaVersao && (
        <div className="mt-3 rounded-lg border border-brand/30 p-3">
          ...
          <textarea value={texto} onChange={(e) => setTexto(e.target.value)} rows={6} className={ta} />
          <div className="mt-2 flex gap-2">
            <button onClick={salvarNovaVersao} disabled={ocupado} className={btnPrim}>Salvar nova versão</button>
            <button onClick={() => { setNovaVersao(false); setTexto(registro.conteudo); }} className={btnSec}>Cancelar</button>
          </div>
        </div>
      )}
          ...
          <ul className="space-y-2">
            {hist.map((h) => (
              <li key={h.id} className="border-b pb-2 last:border-0">
                <p className="text-xs text-ink-soft">v{h.versao} · {h.autor} · {formatarData(h.createdAt)} {h.assinado ? "· assinado" : "· rascunho"}</p>
      ...
      {!editando && !novaVersao && (
        <div className="mt-3 flex flex-wrap gap-2">
          {registro.assinado ? (
            <button onClick={() => { setTexto(registro.conteudo); setNovaVersao(true); }} className={btnSec}><FileStack className="h-4 w-4" /> Nova versão</button>
          ) : (
            <>
